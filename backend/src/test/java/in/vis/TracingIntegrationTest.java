...
@ActiveProfiles("test")
@Import(TracingIntegrationTest.TestTracingConfig.class)
class TracingIntegrationTest {
    ...
    @Test
    void controller_request_produces_at_least_one_span() throws Exception {
        var auth = new UsernamePasswordAuthenticationToken("uid-tracing", null, Collections.emptyList());

        mockMvc.perform(get("/branches").with(authentication(auth)))
                .andExpect(status().isOk());

        assertThat(inMemorySpanExporter.getFinishedSpanItems())
                .as("at least one span must be exported after a controller call (sampling=1.0)")
                .isNotEmpty();
    }
